...
> [!CAUTION]
> **【AIエージェントへの厳格な禁止事項】**
> いかなる理由（軽微な修正、タイポ直し、テストデータの追加等）であっても、**直接 `main` ブランチへコミットおよびPushすることはシステム上固く禁じられています。**（ローカルの git hooks によって技術的にもブロックされます）。
> すべての作業は必ず新しいブランチ（`feature/...`など）を作成し、Pull Requestを経由して行ってください。

### 適用範囲
以下のいずれかに該当する変更は **通常変更** としてフルワークフロー（下記ステップ1〜7）を適用します。

- 動作に影響するコード変更がある
- 複数ファイル（3ファイル以上）にまたがる変更
- 新規ファイル・機能の追加
- 保護対象ファイル（`models.py`, `services.py` 等）への変更

以下のような **軽微な変更** であっても、決して直接 `main` にはコミットせず、同様のフロー（ブランチ作成とPR）を踏んでください。
- Typo・誤字の修正
- ドキュメントへの1〜2行の追記・リンク修正
- フォーマット調整、コメント追加
- `.gitignore` への項目追加
